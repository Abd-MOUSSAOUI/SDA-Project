...
asciinode * build_ascii_tree_recursive(bst_t *t)
{
  asciinode * node;
  ...
  node->left_child = build_ascii_tree_recursive(t->left_child);
  node->right_child = build_ascii_tree_recursive(t->right_child);
...
asciinode * build_ascii_tree(bst_t *t)
{
  asciinode *node;
  if (t == NULL) return NULL;
